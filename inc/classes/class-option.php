...
class Option {
  ...
  private $file;
	...
	protected function setup_hooks() {
		...
		$this->plugin_slug = 'teddybearsprompts';
		$this->plugin_name = 'teddybearsprompts';
		$this->textdomain = 'teddybearsprompts'; // str_replace('_', '-', $plugin_slug);
	...
	public function init() {
		$this->general = (object) wp_parse_args( apply_filters( 'futurewordpress/project/teddybearpopupaddon/settings/general', [] ), [
			'page_title'					=> __( 'Configuration.', 'teddybearsprompts' ),
			'menu_title'					=> __( 'Config', 'teddybearsprompts' ),
			'role'								=> 'manage_options',
			'slug'								=> $this->plugin_slug,
			'page_header'					=> __( 'customization page.', 'teddybearsprompts' ),
			'page_subheader'			=> __( 'Your setting panel from where you can control and customize.', 'teddybearsprompts' ),
			'no_password'					=> __( 'A password is required.', 'teddybearsprompts' ),
		] );
	...
	private function settings_fields() {
		$settings = [];
		$settings = apply_filters( 'futurewordpress/project/teddybearpopupaddon/settings/fields', $settings );
		return $settings;
	}
	...
	public function display_field( $args ) {
		...
		switch( $field['type'] ) {
			...
			case 'radio':
			case 'select_multi':
				$html .= apply_filters( 'futurewordpress/project/teddybearpopupaddon/settings/fields/label', '<br/><span class="description">' . $field['description'] . '</span>', $field );
			break;

			default:
				$html .= apply_filters( 'futurewordpress/project/teddybearpopupaddon/settings/fields/label', '<label for="' . esc_attr( $field['id'] ) . '"><span class="description">' . $field['description'] . '</span></label>' . "\n", $field );
			break;
		}
