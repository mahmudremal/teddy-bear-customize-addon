                    ...
                    if(opt && opt.label) {
                        ...
                        if(!opt.input) {
                            span.innerHTML = opt.label+(
                                (opt?.cost??false)?(
                                ' (+'+(thisClass.config?.currencySign??'$')+''+ parseFloat(opt.cost)+')'
                                ):''
                            );
                        } else {
    ...
    do_pagination: async (plus, thisClass) => {
        ...
        if(await PROMPTS.beforeSwitch(thisClass, plus)) {
            ...
            if(PROMPTS.currentStep >= PROMPTS.totalSteps) {
                ...
                // var popup_body = document.querySelector('.dynamic_popup .popup_body');
                // popup_body.querySelectorAll('input, select').forEach((input)=>{
                //     switch(input.type) {
                //         case 'radio':
                //         case 'checkbox':
                //             if(input.checked) {data.push({name: input.name, item: input.value, image: input.previousElementSibling?.src??'', price: parseFloat(input.dataset?.cost??'0')});}
                //             break;
                //         default:
                //             if(input.name != '' && input.value != '') {data.push({name: input.name, item: input.value, image: input.previousElementSibling?.src??'', price: parseFloat(input.dataset?.cost??'0')});}
                //             break;
                //     }
                // });
                // if(thisClass.voiceRecord?.recordedBlob??false) {
                //     data.push({
                //         name: PROMPTS.i18n?.voice_record??'Voice Record',
                //         value: '',
                //         image: '',
                //         cost: parseFloat(thisClass.voiceRecord.recordButton.dataset?.cost??'0'),
                //         voice: thisClass.voiceRecord.recordedBlob
                //     });
                // }
                // console.log(data);
                ...
                if(submit && submit.classList) {
                    ...
                    formdata.append('dataset', JSON.stringify(PROMPTS.get_formdata(thisClass)));
                ...
                thisClass.Swal.update({currentProgressStep: ((found)?found:(PROMPTS.currentStep-1))});
    ...
    get_formdata: (thisClass) => {
        ...
        Object.keys(form).forEach((name)=>{
            ...
            if(elem.value.trim().toLocaleLowerCase() == form[name].trim().toLocaleLowerCase()) {
                ...
                var match = (PROMPTS.lastJson.product.custom_fields[(split[1]-0)]?.options??[]).find((row)=>row.label && row.label.toLocaleLowerCase()==form[name].toLocaleLowerCase());
                form[name] = {
                    title: elem.value,
                    value: form[name],
                    price: elem.dataset?.cost??0,
                    image: img?.src??(img.dataset?.outfit??''),
                    field: match
                };
            } else {
                console.log(
                    elem.value.trim().toLocaleLowerCase(), form[name].trim().toLocaleLowerCase()
                );
            }
        });
